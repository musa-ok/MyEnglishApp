...
def init_db():
    ...
    try: c.execute("ALTER TABLE users ADD COLUMN target_level TEXT DEFAULT 'B2'")
    except: pass
...
def clean_duplicates():
    ...
    # 1. Words tablosundaki çiftleri sil (En eskisi kalsın, yeniler gitsin)
    c.execute('''
        DELETE FROM words
        WHERE id NOT IN (
            SELECT MIN(id)
    ...
    # 2. Progress tablosundaki çiftleri sil
    c.execute('''
...
def register_user(username, password):
    ...
    try:
        c.execute("INSERT INTO users (username, password, streak, last_login, target_level) VALUES (?, ?, 1, ?, 'B2')", (username, hashed_pw, today))
...
def login_user(username, password):
    ...
    user = c.fetchone()
    if user:
        uid = user[0]; curr_streak = user[4] if user[4] else 0; last_login = user[5]; today = date.today()
        new_streak = curr_streak
        if last_login:
            l_date = datetime.strptime(last_login, "%Y-%m-%d").date()
            if l_date == today - timedelta(days=1): new_streak += 1
            elif l_date < today - timedelta(days=1): new_streak = 1
        else: new_streak = 1
...
def get_quiz_question(user_id, target_levels=None):
    ...
    c.execute(f'''SELECT w.* FROM words w JOIN user_progress up ON w.id = up.word_id WHERE up.user_id = ? AND up.status = 'needs_review' {level_filter} ORDER BY RANDOM() LIMIT 1''', params)
    word = c.fetchone()
...
def inject_ghost_data(username="Ghost"):
    ...
    def safe_insert(u_id, w_id, stat):
        # Bu kullanıcı için bu kelimeye dair HİÇBİR KAYIT YOKSA ekle
    ...
    for eng, tur in learned_list:
        try:
            # Words tablosuna ekle (Zaten varsa eklemez)
            c.execute("INSERT OR IGNORE INTO words (english, turkish, level, pos, example_sentence) VALUES (?, ?, 'A1', 'n.', '-')", (eng, tur))
            c.execute("SELECT id FROM words WHERE english=?", (eng,))
            wid = c.fetchone()[0]
            # Ghost için kontrollü ekle
            safe_insert(user_id, wid, 'learned')
        except: pass

    for eng, tur in review_list:
        try:
            c.execute("INSERT OR IGNORE INTO words (english, turkish, level, pos, example_sentence) VALUES (?, ?, 'B1', 'n.', '-')", (eng, tur))
            c.execute("SELECT id FROM words WHERE english=?", (eng,))
            wid = c.fetchone()[0]
            # Ghost için kontrollü ekle
